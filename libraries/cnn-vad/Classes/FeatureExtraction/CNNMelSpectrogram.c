...
void destroyMelSpectrogram(MelSpectrogram** melSpectrogram){
    MelSpectrogram *in = *melSpectrogram;
    if (in != NULL) {
        if (in->melPower != NULL) {
            free(in->melPower);
            in->melPower = NULL;
        ...
        int nFilt = in->nFilt;
        if (in->melSpectrogramImage != NULL) {
            for (size_t i = 0; i < nFilt; i++) {
                if (in->melSpectrogramImage[i] != NULL) {
                    free(in->melSpectrogramImage[i]);
                    in->melSpectrogramImage[i] = NULL;
                }
            }
            free(in->melSpectrogramImage);
            in->melSpectrogramImage = NULL;
        }
        free(in);
        (*melSpectrogram) = NULL;
    }
